...
- Your raw 32-byte secret (it's revealed *during* the reveal phase, but it never leaves your device before that — Yjs publishes only when you `set()`).
...
- **Network observers** — TLS + WebRTC's DTLS protect the wire, but a determined observer on your local network can see *that* you're playing (room name in SDP exchanges). The room name is not secret.
